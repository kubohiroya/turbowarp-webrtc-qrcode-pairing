...
function checkPolicy() {
  ...
  if (policy.productName !== 'TurboWarp-WebRTC-QRCode-Pairing') {
...
function checkPackageMetadata() {
  ...
  if (packageMetadata.repository?.url !== 'git+https://github.com/kubohiroya/turbowarp-webrtc-qrcode-pairing.git') {
    errors.push('package.json repository.url must point to the current repository');
  }
  if (packageMetadata.bugs?.url !== 'https://github.com/kubohiroya/turbowarp-webrtc-qrcode-pairing/issues') {
...
function checkReadmes() {
  ...
  if (!readmeJa.startsWith('# TurboWarp-WebRTC-QRCode-Pairing\n')) {
...
async function checkPackContents() {
  ...
  if (!files.has('dist/webrtc-qrcode-pairing.js')) {
